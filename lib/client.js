	...
	factory: (require) => {
		...
		function groupFrom(order, store, start, end, partialKey) {
			...
			return {
				...
				error: running === void 0 && entries.some((entry) => entry.error)
			};
		}
		...
		function countSummary(group, t) {
			return [group.reasoningCount > 0 ? t(group.reasoningCount === 1 ? "count.thought" : "count.thoughts", { count: group.reasoningCount }) : "", group.toolCount > 0 ? t(group.toolCount === 1 ? "count.toolCall" : "count.toolCalls", { count: group.toolCount }) : ""].filter(Boolean).join(" · ");
		}
