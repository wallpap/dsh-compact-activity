...
| 运行环境                  | 版本                                 | 结果                                     |
| ------------------------- | ------------------------------------ | ---------------------------------------- |
| DeepSeek Harness 官方 Web | `0.1.2-alpha.1` 客户端发布包         | 兼容；类型检查、jsdom 测试和生产构建通过 |
| DSH Desktop（Windows）    | `2.0.4`，内置 DSH `0.1.2-alpha.1`    | 兼容；profile 组合和 renderer 健康启动通过；类型检查、jsdom 测试和生产构建通过 |
