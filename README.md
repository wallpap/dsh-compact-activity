...
- **保留正文**：下一条模型正文不会被折叠进过程列表。
- **实时显示状态**：执行期间显示 `进行中...`、计数和最后一项活动摘要。
- **失败统计**：过程中的失败步骤单独计数。
- **识别命令退出失败**：除常规工具调用失败外，PWSH、Bash 等终端工具的非零退出码或终止信号也计入失败步骤。
- **终态显示**：最后一个过程项失败或中断时显示 `执行错误`，后续模型正文不会覆盖该状态正常结束时显示`已完成`。
...
| 单条过程消息       | 收进一个默认折叠的总过程项           |
| 正在思考           | 显示 `进行中...`、计数和 `正在思考`(浅紫色)     |
| 正在调用工具       | 显示最后一个官方工具的类型和摘要(浅紫色)        |
| 工作完成           | 显示 `已完成` 和计数，不显示摘要(浅绿色)        |
| 过程中某些工具调用失败但最后工具调用成功 | 显示 `已完成`，并保留失败步骤计数        |
| 最后过程项异常结束 | 显示 `执行错误`，并复用相同的图标计数(浅红色)    |
| 执行错误后输出正文 | 过程组仍显示 `执行错误`，正文保持可见    |
| 下一轮独立工具调用失败 | 独立错误行，不归入上一过程组 |
| 切换 DSH 语言      | 总过程状态和计数同步切换中文或英文      |
| 展开总过程         | 用过程轨道连接原版 Think 和工具组件，并按子项状态显示淡色底板 |
| 单个step中同时有思考+正文 | 只折叠 Think，正文继续显示              |
...
| 运行环境                  | 版本                                 | 结果                                     |
| ------------------------- | ------------------------------------ | ---------------------------------------- |
| DeepSeek Harness 官方 Web | `0.1.3-alpha.1` 客户端发布包         | 源码兼容；类型检查、jsdom 测试和生产构建通过；真实宿主未回归 |
| DSH Desktop（Windows）    | `2.0.5`，内置 DSH `0.1.2-rc.1`      | 复用同一 Web Client 契约；真实 Desktop 启动和 profile 组合未回归 |
