...
让 DeepSeek Harness 的思考、工具调用和图片输出更容易浏览。

插件把连续的思考和工具调用收进一个默认折叠的总过程项。展开后仍使用 DSH 官方组件，模型正文保持可见。

## 功能概览

- **过程折叠**：连续或单条思考、工具调用都可以收进一个总过程项。
- **实时状态**：显示进行中、已完成或执行错误，以及思考、工具和失败步骤计数；运行中的工具会复用官方摘要。
- **错误识别**：统计结构化工具错误、思考中断，以及 Bash/PWSH 等终端工具的非零退出码和终止信号。
- **正文保留**：同一 step 同时包含思考和正文时，只折叠 Think，正文继续显示。
- **图片折叠**：非用户输入的图片默认收起，用户和 steering 消息中的图片保持原样。
- **官方交互不变**：不替换官方渲染器，不修改模型上下文、会话日志或工具执行。

状态、计数和图片文案跟随 DSH 的语言设置，支持 `中文` 和 `English`。切换语言后，已显示的总过程项和图片标记会同步更新。
...
插件没有独立的设置页面。安装后重启 DSH Web 或 Desktop 即可生效；语言由 DSH 的 Language 设置控制：

| 场景 | 显示行为 |
| ---- | -------- |
| 连续或单条过程消息 | 合并为一个默认收起的总折叠项 |
| 正在思考 | 显示 `进行中...`、计数和 `正在思考` |
| 正在调用工具 | 显示最后一个官方工具的类型和摘要 |
| 正常完成 | 显示 `已完成` 和计数；历史失败保留在失败计数中 |
| 最后过程项失败或中断 | 显示 `执行错误`；后续正文不会覆盖该状态 |
| 同一 step 同时有思考和正文 | 只折叠 Think，正文继续显示 |
| 展开总过程 | 继续使用官方 Think 和工具组件，并显示成员状态 |
| 切换 DSH 语言 | 总过程和图片标记同步切换中文或英文 |
...
截至 2026-09-19，以下结果基于本地源码和 Desktop 内置运行时包；尚未启动真实 Electron/Desktop 窗口做人工回归。

| 运行环境 | 版本 | 结果 |
| -------- | ---- | ---- |
| DeepSeek Harness 官方 Web | `0.1.6-alpha.2`（commit `ddefc45f`） | 插件开发依赖直接链接本地源码；`npm run typecheck`、`npm test` 和 `npm run build` 通过。真实 Web 窗口未做人工回归。 |
...
用户输入和 steering 消息中的图片保持原样。插件使用自己的独立标记：`data-dca-image-group`、`data-dca-image-target` 和 `data-dca-image-hidden`，只插入原生 `<details>/<summary>` 标记并隐藏原图片目标，不移动或替换 DSH 的图片 DOM 子树。

图片折叠支持淡入淡出过渡，并遵循 `prefers-reduced-motion`。检测复用 DSH 的图片按钮、Markdown 图片属性和失败回退结构；同一 Markdown 图片引用只生成一个标记，展开后显示图片名称和路径。Windows Desktop 中的本地 Markdown 路径会根据当前 Session 工作目录补全盘符，并通过已鉴权的 `/api/file` 路由重新挂载；无法解析或文件不存在时保留 DSH 的回退文本。升级 DSH 后，建议检查 Assistant 图片、工具图片、多图画廊、失败回退、用户图片和 lightbox 展开。
